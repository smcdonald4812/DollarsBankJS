...
class Savings extends React.Component {
    ...
    componentDidMount() {
        TransactionService.get(this.props.user.id, 'SAVINGS').then((response) => {
            ...
            <div className="container mr-auto" style={{ marginTop: "100px"}}>
                <Form onSubmit={this.handleSubmit}>
                    <Table striped bordered hover borderless>
                        <tbody>
                            <tr><td>{this.props.user.firstname}'s</td><td>Savings Account</td><td>Account Number</td><td>{this.state.account.id}</td></tr>
